...
$(document).ready(function(){
			$.ajax({
				...
				success : function(response){
					...
					for(i=0;i<data.length;i++)
					{
						...
						var opt = '<div class="col-12"><form class="delete-shortcut-form" id="'+data[i].id+'">@csrf<div class="row my-3 py-2 shadow-sm rounded-lg"> <div class="col-1 text-center pt-2">'+data[i].s_number+'</div>  				<div class="col-6"><input type="text" name="name" class="form-control outline-none shortcut-name-input" value="'+data[i].title+'"></div>  				<div class="col-2">  					<button class="btn btn-danger delete-shortcut-btn" id="'+data[i].id+'" type="submit" sk="delete"><i class="fa fa-trash" ></i>  Delete</button>  				</div>  				<div class="col-3">  					<button class="btn btn-primary update-shortcut-btn" id="'+data[i].id+'" type="submit" sk="update"><i class="fa fa-edit"></i> Update</button>  				</div>  			</form></div>';
	...
	$(".delete-shortcut-form").each(function(){
		...
		if($(submiter).attr('sk') == $(delete_btn).attr('sk'))
		{
			...
			$.ajax({
			type : 'delete',
			url : 'http://127.0.0.1:8000/api/shortcuts/'+id,
			data : {
				_token : $("body").attr('token')
			},
			beforeSend : function(){
